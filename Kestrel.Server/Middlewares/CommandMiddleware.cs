...
public sealed class CommandMiddleware(IEnumerable<IAsyncCommand> commands)
    : IApplicationMiddleware<CommandContext>
{
    private readonly ReadOnlyDictionary<CommandType, IAsyncCommand> _commands = new(commands.ToDictionary(
        item => item.CommandType,
        item => item));

    async ValueTask IApplicationMiddleware<CommandContext>.InvokeAsync(
        ApplicationDelegate<CommandContext> next,
